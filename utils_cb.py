...
def simple_binding_curve(x, Kd, Bmax):
    """
    Simple binding curve (Michaelis-Menten like)
    Kd = dissociation constant (similar to EC50)
    Bmax = maximum binding
    """
    return (Bmax * x) / (Kd + x)

def fit_simple_binding(concentrations, fluorescence):
    """
    Fit a simple binding curve to 4 data points
    """
    # Convert to numpy arrays
    x = np.array(concentrations)
    y = np.array(fluorescence)
    
    # Initial guesses
    Bmax_guess = np.max(y) * 1.5  # Slightly above max observed
    Kd_guess = x[np.argmin(np.abs(y - np.max(y)/2))]  # Conc at half-max
    
    try:
        # Fit the curve
        params, covariance = curve_fit(
            simple_binding_curve, 
            x, y,
            p0=[Kd_guess, Bmax_guess],
            bounds=(0, np.inf)  # Keep parameters positive
        )
        
        Kd, Bmax = params
        
        # Calculate R-squared
        y_pred = simple_binding_curve(x, *params)
        ...
        return {
            'Kd': Kd,
            'EC50': Kd,  # For binding curves, Kd ≈ EC50
            'Bmax': Bmax,
            'R_squared': r_squared,
            'params': params
        }
...
def hill_equation(x, Bmax, EC50, n):
    """
    Hill equation (simplified logistic)
    Bmax = maximum binding
    EC50 = concentration at half-maximum
